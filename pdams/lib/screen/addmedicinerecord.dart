...
class AddMedicineRecordScreen extends StatefulWidget {
  final Doctor user;
  const AddMedicineRecordScreen({super.key, required this.user});

  @override
  _MedicineRecordScreenState createState() => _MedicineRecordScreenState();
}

class _MedicineRecordScreenState extends State<AddMedicineRecordScreen> {
  ...
  void addMedicineRecord() {
    ...
    String? doctorID = widget.user.doctorID;
    ...
    if (patientICNumber == widget.user.icNumber) {
      ...
      _fetchPatientName(patientICNumber!).then((patientName) {
        if (patientName != null) {
          http.post(
            Uri.parse("https://vppdams.000webhostapp.com/add_medicine.php"),
            // Uri.parse("${MyConfig().SERVER}/pdams/php/add_medicine.php"),
            body: {
              'name': name,
              'icNumber': icNumber,
              'doctorID': doctorID,
